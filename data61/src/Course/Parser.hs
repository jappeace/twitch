...
-- | Return a parser that succeeds with a character off the input or
--  fails with an error if the input is empty.
--
-- >>> parse character "abc"
-- Result >bc< 'a'
--
...
instance Functor Parser where
  ...
  (<$>) mapf (P fun) = P $ \x -> mapf <$> fun x

-- | Return a parser that always succeeds with the given value and
--   consumes no input.
--
...
(|||) (P parseFun) (P parseFun2) = P $ \x ->
  let res1 = (parseFun x) in
  if isErrorResult res1
  then parseFun2 x
  else res1
...
--
--   * if that parser succeeds with a value (a),
--    put that value into the given function
...
instance Monad Parser where
  ...
  (=<<) nextF parsa = P $ \inx  -> case parse parsa inx of
    Result nextVal x    -> parse (nextF x) nextVal
    UnexpectedEof       -> UnexpectedEof
    ExpectedEof d       -> ExpectedEof d
    UnexpectedChar d    -> UnexpectedChar d
    UnexpectedString  d -> UnexpectedString d
...
satisfy predF = do
  cherRes <- character
  if predF cherRes then pure cherRes else unexpectedCharParser cherRes
...
list parser = P $ \x -> eat parser x Nil

eat :: Parser a -> Input -> List a -> ParseResult (List a)
eat parser inp prev = case parse parser inp of
    Result d a -> eat parser d (a :. prev)
    d          -> pure <$> d
